...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.contentViewWithIndex = ^(UMSocialShareScrollView *scrollView, NSInteger index, DJXScrollSubViewLocation location){
            ...
            pageSize.width = pageSize.width - weakSelf.edgeInsets.left - weakSelf.edgeInsets.right;
            ...
            NSLog(@"page views are %@", weakSelf.pageViews);
            return pageView;
        };
    }
    return self;
...
- (void)resetSelfFrame
{
    ...
    self.columnCount = (selfFrame.size.width - self.edgeInsets.left - self.edgeInsets.right + self.expectLineSpace) / (self.expectLineSpace + self.itemSize.width);
    ...
    if (self.columnCount > 1) {
        self.columnSpace = (selfFrame.size.width - self.edgeInsets.left - self.edgeInsets.right - self.columnCount*self.itemSize.width)/(self.columnCount - 1);
    }
    ...
    //设置偏移
    selfFrame.origin.y = superView.frame.size.height - selfFrame.size.height - self.cancelButton.frame.size.height - UMSocial_MenuAndCancel_Space - self.pageControl.frame.size.height;
...
- (void)pageView:(UIView *)pageView items:(NSArray *)items size:(CGSize)size
{
    ...
    pageViewFrame.origin.x = self.edgeInsets.left;
...
- (CGRect)itemFrameWithIndex:(NSInteger)index
{
    ...
    rect.origin.x =  column * (self.columnSpace + self.itemSize.width);
...
- (void)show
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        CGRect frame = self.frame;
        frame.origin.y = self.superview.frame.size.height - frame.size.height - self.cancelButton.frame.size.height - UMSocial_MenuAndCancel_Space - self.pageControl.frame.size.height;
        self.frame = frame;
